...
parser.add_argument('input', action="store", default=None, type=str, nargs='+',
                    help="List of input files separated by spaces. Either .pdb or .fasta files (at least one of each required)")
...
def get_merge_df(uni_dict, pdb_dict):
    ...
    for uni_file, uni_short_to_full_dict in uni_dict.items():
        df = pd.DataFrame({'UniprotID': list(uni_short_to_full_dict.keys()), 'UniprotID_Full': list(uni_short_to_full_dict.values())})
        df['FastaFile'] = uni_file
        df_uni_list.append(df)
    for pdb_file, chain_id_to_uni_dict in pdb_dict.items():
        df = pd.DataFrame({'UniprotID': list(chain_id_to_uni_dict.values()), 'PDB_ChainID': list(chain_id_to_uni_dict.keys())})
        df['PDBFile'] = pdb_file
...
def main():
    ...
    assert pdb_file_list or not fasta_file_list, "Either no fasta or pdb file found in input; the script needs at least one of each"
