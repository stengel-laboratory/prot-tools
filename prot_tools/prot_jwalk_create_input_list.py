...
def write_out_files(df):
    ...
    df_n = pd.DataFrame()
    df_n['tmp'] = df['AbsPos1'].astype(int).astype(str) + '|' + df['PDB_ChainID1'] + '|' + df['AbsPos2'].astype(
        int).astype(str) + '|' + df['PDB_ChainID2']
    for model in models:
...
def main():
    ...
    for inp in args.input:
        ...
        if 'PDBFile' in df_tmp.columns:
            ...
            exit(1)
    assert chain_df_list and xquest_df_list, "Either no chain match or xquest file found in input; the script needs " \
